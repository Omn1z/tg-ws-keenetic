...
**Ветка `rust` готовит выпуск 2.1.0. Команда установки из последнего релиза
...
Текущая версия встроена в бинарник из `Cargo.toml` при сборке. Например,
`v2.1.0` в панели может отображаться ещё до публикации релиза `v2.1.0`.
Релизный тег должен совпадать с версией сборки с префиксом `v`; следующему
...
```sh
sh /tmp/tgws-install.sh --version v2.1.0
```
