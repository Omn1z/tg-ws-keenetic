...
- **Небольшая панель**: ссылка Telegram, трафик, настройки, секрет, пароль и
  ...
  шрифтов, графических библиотек и npm. Статистика опрашивается раз в 5 секунд,
  только в видимой вкладке. Состояние
...
`Cargo.lock`, создайте тег `v2.1.0` (или следующую версию) на проверенном коммите
...
Версия в панели берётся из `Cargo.toml` при сборке: сейчас `2.1.0` отображается
как `v2.1.0`. Это версия установленного бинарника, а не свидетельство публикации
релиза. Для выпуска этой сборки тег должен называться `v2.1.0`; для следующего
обновления увеличьте версию и создайте соответствующий тег. Пока подходящий
Rust-релиз не опубликован, панель не сможет установить его.
